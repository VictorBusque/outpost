"""Pydantic v2 models for ``sow.yaml``.
...
``exposure.hosts`` ⊆ routed hosts) are enforced on :class:`sowConfig` after
the whole tree is parsed.

Config *loading* (YAML → model) and *digest* computation live in
``sow/config.py``; this module stays pure schema.
"""
...
from sow.constants import DEFAULT_PORT_RANGE, NGINX_PORT
...
# empty before first deploy (seeded once by ``apply``). sow never verifies
...
class sowConfig(BaseModel):
    """The root config model: the whole ``sow.yaml`` tree."""
