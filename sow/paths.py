...
Everything sow owns on disk lives under two XDG roots (``rfc.md`` §6):
the runtime tree ``~/.local/share/sow/`` (generated configs, clones, data,
...
def _runtime_default() -> Path:
    return Path.home() / ".local" / "share" / "sow"
...
class RuntimePaths:
    """The two on-disk roots sow writes under, plus derived paths.

    ``base`` is the runtime root (``~/.local/share/sow``); ``user_units`` is
