...
_REPO_ROOT = Path.home() / ".local" / "share" / "sow" / "repos"
_DATA_ROOT = Path.home() / ".local" / "share" / "sow" / "data"
...
def compute_facts(config: sowConfig, ports: Mapping[str, int]) -> dict[str, dict[str, str]]:
    """Build the ``{service -> {ADDRESS, PORT, DATA_DIR}}`` fact table.

    ``ports`` is the allocator output from :func:`~sow.engine.ports.allocate_all`.
...
def build_spec(
    ...
    *,
    ...
    return UnitSpec(
        description=f"sow service: {name}",
...
class NginxServerSpec:
    """One ``server`` block, computed from a :class:`~sow.models.Route`.
...
def build_nginx_specs(config: sowConfig, ports: Mapping[str, int]) -> list[NginxServerSpec]:
...
def render_nginx(config: sowConfig, ports: Mapping[str, int]) -> str:
...
class CloudflaredSpec:
    ...
    operator's credentials JSON (prd.md: "sow renders a cloudflared config
...
def render_cloudflared(config: sowConfig, tunnel: str) -> str:
    ...
    is not part of ``sow.yaml`` and so cannot be derived from ``config``.
    """
