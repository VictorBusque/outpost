"""The apply pipeline — the core of sow's single loop.
...
The whole pipeline is driven through the :class:`~sow.sysdeps.run.Runner`
seam and injectable :class:`~sow.paths.RuntimePaths`, so it is fully
...
from sow.engine.health import HealthCheckError, wait_for
    ...
    *,
    ...
    try:
        ...
        staging = (
            ...
            else Path(tempfile.mkdtemp(prefix="sow-stage-"))
        )
...
def _materialize(
    config: sowConfig, runner: Runner, paths: RuntimePaths, cfg_path: Path
) -> tuple[sowConfig, dict[str, str]]:
...
def _maybe_build(runner: Runner, svc: Service, repo_dir: Path, sha: str) -> None:
    ...
    The marker (``repos/<service>/.sow/built.sha``) records the sha last
...
def _read_tunnel(config: sowConfig) -> str:
    """Read the cloudflared ``TunnelID`` from the operator's credentials file.

    The tunnel UUID is operator-owned (not in ``sow.yaml``), so the apply
...
def _all_active(runner: Runner, config: sowConfig, paths: RuntimePaths) -> bool:
...
def _sync_symlinks(config: sowConfig, paths: RuntimePaths) -> None:
    ...
    non-sow units); add explicit cleanup if it becomes a problem.
    """
