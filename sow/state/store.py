...
:func:`~sow.state.io.write_atomic`.
"""
...
DEFAULT_STATE_PATH = Path.home() / ".local" / "share" / "sow" / "state.json"
...
def _from_dict(raw: dict[str, object], path: Path) -> State:
    ...
    a newer sow that added a key must not crash an older one reading it back,
