...
``prd.md`` §169, sow "reloads via ``systemctl --user reload``" — never
...
:class:`~sow.sysdeps.run.SubprocessError`.
"""
...
from sow.sysdeps.systemctl import daemon_reload

__all__ = ["NGINX_UNIT", "SubprocessError", "daemon_reload", "reload", "test"]

# The user NGINX unit name. The docs never literally pin the string; "sow-nginx"
